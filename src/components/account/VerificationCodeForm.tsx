...
const VerificationCodeForm: React.FC<Props> = ({ email, onSuccess, onRequestVerificationCode }) => {
  ...
  return (
    <div className="flex flex-col border rounded-xl border-stone 100 shadow-md p-6 mt-8 w-2/5">
      ...
      {showTooltip && (
          <div className={`fixed top-96 flex transform -translate-y-full ${message.includes('error') ? 'bg-red-500' : 'bg-sky-600'} text-white py-2 px-4 rounded-md transition-all duration-300 ease-in-out`}>
            {message}
          </div>
        )}
