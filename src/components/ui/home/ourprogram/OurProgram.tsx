...
const OurProgram = () => {
  ...
  return (
    <section
      className={`relative flex flex-col w-full sm:px-32 sm:py-20 p-6 bg-${theme}-hi`}
    >
      <div className="flex sm:flex-row flex-col sm:pb-20 pb-12">
        ...
        >
          Welcome to <span className="text-sky-500">Human Initiative</span> Platform to help everyone
        </h5>
      ...
      >
        {programCard.map((program, index) => (
          <div
            key={index}
            className={`rounded-xl text-${theme} sm:pb-0 pb-6`}
            data-aos="fade-up"
            data-aos-easing="linear"
            data-aos-duration="1500"
          >
            <div className={`bg-program-${theme}-card p-4 rounded-xl`}>
            <div className={`pb-4`}><span className={`text-4xl text-sky-500`}>{program.icon}</span></div>
            <h3 className={`text-${theme}-hi sm:text-xl text-lg font-semibold sm:pb-6 pb-3`}>{program.label}</h3>
            <p className={`text-${theme}-hi `}>{program.text}</p>
            <Link href={program.url} className={`text-${theme}-hi inline-block bg-transparent text-2xl p-1`}>
              <HiArrowUpRight/>
            </Link>
            </div>
          </div>
