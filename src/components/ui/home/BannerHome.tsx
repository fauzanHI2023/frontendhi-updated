      ...
      <div className="flex sm:flex-row flex-col w-full">
        <div className="flex flex-col sm:w-1/2 w-full justify-center sm:pb-0 pb-8" data-aos="fade-left">
          <h3 className="font-bold sm:text-5xl text-2xl sm:pb-10 pb-3">
            Helps <span className="font-black text-blue-800">connect</span> those who care with those who need help.
          </h3>
        ...
        <div className="flex sm:w-1/2 w-full items-center justify-center" data-aos="fade-right">
          <TabGroup className="w-2/3">
            <TabList className="w-full mb-4">
              <Tab className="w-2/4 rounded-tl-[23px] rounded-bl-[23px] py-3 text-center dark:bg-white bg-blue-800 dark:data-[selected]:bg-blue-950 data-[selected]:bg-white dark:text-blue-900 text-white dark:data-[selected]:text-white data-[selected]:text-blue-900">Donasi Sekali</Tab>
              <Tab className="w-2/4 rounded-tr-[23px] rounded-br-[23px] py-3 text-center dark:bg-white bg-blue-800 dark:data-[selected]:bg-blue-950 data-[selected]:bg-white dark:text-blue-900 text-white dark:data-[selected]:text-white data-[selected]:text-blue-900">Donasi Perbulan</Tab>
            </TabList>
            <TabPanels>
              <TabPanel className="flex flex-col text-sm">
                <div className="dark:bg-blue-950 bg-blue-800 dark:text-white text-white p-4 text-center rounded-tl-[23px] rounded-tr-[23px]">
                  <h4>Pilih jumlah yang akan diberikan per bulan</h4>
                </div>
                <form className="flex flex-wrap dark:bg-white bg-white p-4">
                  List Produk disini
                  <button type="submit" className="dark:text-white text-white w-full dark:bg-blue-950 bg-blue-800 rounded-[50px] py-3 px-4 text-center">
                    Donasi Sekarang
                  </button>
                </form>
                <div className="flex flex-col dark:bg-blue-950 bg-blue-800 dark:text-white text-white p-8 text-center rounded-bl-[23px] rounded-br-[23px]">
                  <h4>Donasi bulanan anda sebesar 10.000 memberikan bantuan kepada masyarakat yang membutuhkan.</h4>
                </div>
              </TabPanel>
              <TabPanel className="flex flex-col text-sm">
                <div className="dark:bg-blue-950 bg-blue-800 dark:text-white text-white p-4 text-center rounded-tl-[23px] rounded-tr-[23px]">
                ...
                <form className="flex flex-wrap gap-x-3 dark:bg-white bg-white p-4" onSubmit={handleDonateNow}>
                  {loading ? (
                      ...
                      products.map((product) => (
                        <div key={product.product_id} className="w-1/4 mb-4">
                          <input type="radio"
                              ...
                              onChange={() => setSelectedProductId(product.product_id)}
                              className="peer hidden"
                            />
                          <label htmlFor={product.product_id} className="flex flex-col items-center w-full cursor-pointer p-2 text-center rounded-3xl bg-slate-300 text-slate-600 peer-checked:bg-blue-800 peer-checked:text-white">
